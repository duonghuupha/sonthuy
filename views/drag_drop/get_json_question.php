...
foreach($target as $row_t){
    $array_target[] = '{"text": "'.$row_t['title'].'", "accept": "'.$row_t['id_temp'].'", "file": "'.$row_t['file'].'", "lesson_id": "'.$item[0]['lesson_id'].'"}';
}
foreach($answer as $row_a){
    $array_answer[] = '{"text": "'.$row_a['title'].'", "image": "'.$row_a['file'].'", "match": "'.$row_a['target_id'].'", "lesson_id": "'.$item[0]['lesson_id'].'"}';
}
echo '{
