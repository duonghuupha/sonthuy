var url;
$(function(){
    var gwdth = $('#list_cate').width(), fwdth = $('.full').width();
    $('#list_cate').jqGrid({
...
function format_image(cellvalue, options, rowObject){
    var html = '';
    if(cellvalue != ''){
        html += '<img src="'+baseUrl+'/public/test/cate/'+cellvalue+'" style="height: 50px;"/>';
    }else{
...
function change(status, idh){
    ...
    del_data(data_str, "Bạn có chắc chắn muốn cập nhật trạng thái cho bản ghi này?", baseUrl + '/test_cate/change', '#list_cate', baseUrl + '/test_cate?token='+localStorage.getItem('token'));
}
...
function del(){
    ...
    if(rowKey == null){
        ...
        del_data(str_data, "Bạn có chắc chắn muốn xóa bài giảng này không?", baseUrl + '/test_cate/del', "#list_cate", baseUrl + '/test_cate?token=' + localStorage.getItem('token'));
    }
}
