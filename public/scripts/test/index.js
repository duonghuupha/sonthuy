...
$(function(){
    var gwdth = $('#list_test').width();
    $('#list_test').jqGrid({
        url: baseUrl + '/test/json?token='+localStorage.getItem('token'),
        datatype: "json",
        mtype: "GET",
        colModel: [
            {label: 'Mã câu hỏi', name: 'code', hidden: true},
            {label: 'Danh mục', name: 'test_cate_title', width: 150, align:"center"},
            ...
            {label: 'Độ khó', name: 'level', width:70, align:"center", formatter: format_level},
...
function format_level(cellvalue, options, rowObject){
    if(cellvalue == 1){
        return 'Dễ';   
    }else if(cellvalue == 2){
        return 'Bình thường';
...
function update(){
    reset_form('#fm'); $('#form_type').empty();
    var rowKey = $('#list_test').jqGrid('getGridParam',"selrow");
    if(rowKey == null){
        ...
        url = baseUrl + '/test/update?token='+localStorage.getItem('token')+"&id="+row.id;
    }
}
...
function del(){
    ...
    if(rowKey == null){
        ...
        del_data(data_str, "Bạn có chắc chắn muốn xóa câu hỏi này?", baseUrl + '/test/del', '#list_test', baseUrl + '/test/json?token='+localStorage.getItem('token'));
    }
}

function change(status, idh){
    var data_str = "token="+localStorage.getItem('token')+'&id='+idh+'&status='+status;
    del_data(data_str, "Bạn có chắc chắn muốn cập nhật trạng thái cho câu hỏi này?", baseUrl + '/test/change', '#list_test', baseUrl + '/test/json?token='+localStorage.getItem('token'));
}
...
function save(){
    ...
    if(allRequired){
        ...
        if(required_all){
            ...
            save_form_modal('#fm', url, '#modal-form', '#list_test',  baseUrl+'/test/json?token='+localStorage.getItem('token'));
        }else{
...
function close_modal_match(){
    if(myData_match.length == 0){
        ...
        $.getJSON(baseUrl + '/match/remove_file_temp_after_click_close_modal',
            {token: localStorage.getItem('token'), data: btoa(JSON.stringify(myData_match)), type: 'test'}, 
            function(result){
            if(result.success){
...
function close_modal_drag_drop(){
    if(myData_drag_drop_answer.length == 0 || myData_drag_drop_target == 0){
        ...
        $.getJSON(baseUrl + '/drag_drop/remove_file_temp',
            {token: localStorage.getItem('token'), data: btoa(JSON.stringify(myData_drag_drop_answer)), type: 'test'}, function(result){}
        );
        $.getJSON(baseUrl + '/drag_drop/remove_file_temp',
            {token: localStorage.getItem('token'), data: btoa(JSON.stringify(myData_drag_drop_target)), type: 'test'}, function(result){});
